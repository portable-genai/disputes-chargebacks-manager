...
Disputes and Chargebacks Manager (F2): Deterministic dispute and chargeback lifecycle, intake and representment for BFSI ops.
Rendered from `hex-service-template`, so it starts at reference parity rather than converging
toward it. Package `disputes_chargebacks_manager`, environment prefix
`DISPUTES`, region `asia-southeast1`.

## Documents

[`docs/practices-audit.md`](docs/practices-audit.md) records the per-check verdict against the
common base practices, and it is the file to update when a gap closes. `docs/runbook.md` and
`docs/onprem-migration.md` are the operational pair.
...
- `src/disputes_chargebacks_manager/domain/` is PURE stdlib. No web framework, no cloud SDK,
...
- **The exposure guard is derived from the IDENTITY BINDING, and from nothing else.** An end-user
  ...
  `DISPUTES_S2S_TOKEN` may never enter that decision: it authenticates a
...
- **The one adapter that declares `VERIFIED` must EARN it, and it is the one adapter that may not
  ...
  configured `DISPUTES_IAP_AUDIENCE`, three-state: unset or emptied REFUSES,
...
- **One profile read, three states, refused at import.** `config.PROFILE_CHOICE` resolves once at
  ...
  `DISPUTES_PROFILE` (mentioning it in a refusal message is wanted, reading
