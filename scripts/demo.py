...
from disputes_chargebacks_manager.domain import (
    kernel,
    models,
)
from disputes_chargebacks_manager.domain.pii import (
    JURISDICTIONS,
)
...
SERVICE_NAME = "Disputes and Chargebacks Manager"
CATALOG_ID = "F2"
REPOSITORY = "disputes-chargebacks-manager"
...
#: than trusting the pattern pack to agree with itself. It matches the synthetic transcript the
#: local conversation channel serves for ``conv-demo-pii``.
PLANTED_NRIC = "S1234567D"

_AS_OF = date(2025, 6, 1)

#: A routine, ELIGIBLE card-scheme dispute (filed within its window): decided, not routed.
ROUTINE_DISPUTE = models.Dispute(
    id="DSP-DEMO-OK",
    tenant=TENANT,
    track=models.DisputeTrack.CARD_SCHEME,
    reason_code="10.4",
    amount_minor=42_000,
    currency="SGD",
...
#: A high-abuse RETAIL dispute paired with an abusive history: the engine DENIES and routes (R8).
ABUSE_DISPUTE = models.Dispute(
    id="DSP-DEMO-ABUSE",
    tenant=TENANT,
    track=models.DisputeTrack.RETAIL,
...
ABUSE_HISTORY = models.CustomerHistory(
    dispute_count_90d=9, prior_abuse_count=1, refund_total_minor_90d=300_000
)

#: The intake conversation the redaction beat classifies. It is a regulatory complaint carrying a
#: planted national id, so it both fails closed to review (routed) and exercises redaction.
PII_CONVERSATION = "conv-demo-pii"
...
STEPS: tuple[Step, ...] = (
    ...
    Step(
        key="routine",
        label="An eligible dispute: decided, cited, advanced (NOT routed)",
        narration=(
            "A card-scheme dispute filed inside its reason-code window. The eligibility verdict "
            "comes from deterministic code, never from a model, and the case advances to evidence "
            "review. Nothing is routed, because manufacturing a review trains rubber-stamping."
        ),
    ),
    Step(
        key="escalation",
        label="A refund-abuse DENY: consequential AND routed (rule R8)",
        narration=(
            "A high-abuse dispute. The transparent additive score crosses the deny band, so the "
            "verdict is consequential: setting the flag is not the escalation, routing is. It is "
            "handed to the maker-checker console with dual control, in the same call."
        ),
    ),
    Step(
        key="redaction",
        ...
        narration=(
            "An in-channel intake with a national id planted in the transcript. It classifies as "
            "a regulatory complaint, fails closed to human review, and the identifier is masked "
            "before anything is written: the immutable record never contains it."
        ),
    ),
    Step(
        key="review_queue",
...
class DemoRun:
    ...
    def _step_routine(self) -> Produced:
        return self._open_panels(ROUTINE_DISPUTE, expect_routing=False)

    def _step_escalation(self) -> Produced:
        return self._abuse_panels(ABUSE_DISPUTE, ABUSE_HISTORY, expect_routing=True)

    def _step_redaction(self) -> Produced:
        result = self.service.intake(PII_CONVERSATION, tenant=TENANT, actor=ACTOR)
        self.cases += 1
        ...
        classify = Panel(
            title="Intake classification: " + PII_CONVERSATION,
            rows=(
                Row("Category (closed set)", result.classification.category.value),
                Row("Opened a lifecycle case", str(result.opened)),
                Row("Routed to human review", review_ref or "not routed", "ok"),
            ),
            note=(
                "The model classifies into a CLOSED set only; a regulatory complaint fails closed "
                "to human review and never opens a card/retail lifecycle case."
            ),
        )
        redact = Panel(
            title="Redact before the write",
            rows=(
                Row("Identifier in the submitted transcript", PLANTED_NRIC, "warn"),
                Row(
            ...
            note=(
                ...
                "Masking happens on the way in, before the audit record and before the console."
            ),
    ...
    def _open_panels(self, dispute: models.Dispute, *, expect_routing: bool) -> Produced:
        result = self.service.open_dispute(dispute, actor=ACTOR, as_of=_AS_OF)
        self.cases += 1
        review_ref = result.review_ref
        if result.disposition.requires_human_review:
            self.escalated += 1
            self.routed += 1
        consistent = bool(review_ref) == expect_routing
        decision = Panel(
            title="Eligibility: " + dispute.id,
            rows=(
                Row("Track / reason code", dispute.track.value + " / " + dispute.reason_code),
                Row("Eligible", str(result.eligibility.eligible), "ok"),
                Row("Case state", result.case.state.value),
                Row(
                    "Routed to review",
                    review_ref or "not routed (eligible, advanced)",
                    "ok" if consistent else "bad",
                ),
                Row("Attributed to", ACTOR),
            ),
            note=(
                "The eligibility verdict is computed by pure stdlib code and is replayable. A "
                "model would narrate this result; it never produces it."
            ),
        ...
        evidence = Panel(
            ...
            rows=tuple(
                ...
                for citation in result.eligibility.citations
            )
        ...
        facts = {
            ...
            "requires_human_review": result.disposition.requires_human_review,
            "review_ref": review_ref,
    ...
    def _abuse_panels(
        self, dispute: models.Dispute, history: models.CustomerHistory, *, expect_routing: bool
    ) -> Produced:
        decision = self.service.assess_abuse(dispute, history, actor=ACTOR)
        self.cases += 1
        review_ref = decision.review_ref
        consequential = decision.disposition is not None
        if consequential:
            self.escalated += 1
            self.routed += 1
        consistent = bool(review_ref) == expect_routing == consequential
        assessment = decision.assessment
        verdict = Panel(
            title="Refund-abuse decision: " + dispute.id,
            rows=(
                Row("Outcome", assessment.outcome.value, "bad" if consequential else "ok"),
                Row("Score (transparent additive)", str(assessment.score)),
                Row("Signals", ", ".join(assessment.signals)),
                Row(
                    "Routed to review",
                    review_ref or "not routed",
                    "ok" if consistent else "bad",
                ),
                Row("Attributed to", ACTOR),
            ),
            note=(
                "The score and the verdict are deterministic and replayable; a DENY routes to "
                "maker-checker sign-off with dual control. A model never produces the verdict."
            ),
            tone="ok" if consistent else "bad",
        )
        evidence = Panel(
            ...
            or (Row("citations", "NONE", "bad"),),
            note="Every firing signal cites the datum that fired it.",
        )
        facts = {
            "dispute_id": dispute.id,
...
def _exit_audit(container: Any) -> Any:
    return container.audit.record(
        kernel.AuditEvent(
            ...
            redacted_summary="DSP-DEMO (FICTIONAL): ineligible rejection",
        )
    )
...
def _exit_review(container: Any) -> Any:
    ...
    return container.review_router.route(
        models.DisputeDisposition(
            subject=ROUTINE_DISPUTE.id,
            severity=kernel.Severity.HIGH,
            decision=kernel.Decision.ESCALATED,
            summary="DSP-DEMO (FICTIONAL): ineligible rejection requires sign-off",
            requires_human_review=True,
            dispute_id=ROUTINE_DISPUTE.id,
            citations=(citation,),
        ),
        maker=ACTOR,
        tenant=TENANT,
...
def _exit_case_engine(container: Any) -> Any:
    return container.case_engine.open_case(
        ROUTINE_DISPUTE, CARD_SCHEME_WORKFLOW, opened_on=ROUTINE_DISPUTE.intake_date
    )


def _exit_document_extraction(container: Any) -> Any:
    return container.document_extraction.extract_raw(
        "proof_of_delivery: tracking SG9988", doc_type="chargeback_evidence", document_id="EV-DEMO"
    )


def _exit_conversation_channel(container: Any) -> Any:
    return container.conversation_channel.fetch_turns("conv-unauth-001")


def _exit_regulator_response(container: Any) -> Any:
    return container.regulator_response.draft_response(
        dispute_id="DSP-DEMO", category="complaint", redacted_narrative="a complaint narrative"
    )


def _exit_narration(container: Any) -> Any:
    return container.narration.narrate(instruction="Summarise.", facts=(("reason_code", "10.4"),))
...
#: The calls the exit profile must REFUSE, one per declared port. Add a port, add a row: a seam
#: nobody calls is a seam nobody knows is unimplemented, and ``portability_demo.py`` fails the
#: build if any declared port has no exit call here.
#:
...
EXIT_CALLS: dict[str, Callable[[Any], Any]] = {
    ...
    "document_extraction": _exit_document_extraction,
    "conversation_channel": _exit_conversation_channel,
    "regulator_response": _exit_regulator_response,
