...
| `drop_ui.py` | Removes `ui/` together with its npm dependabot ecosystem, in one consistent step (`make drop-ui`). It used to remove a `ui-gate` workflow too; that workflow was retired with the rest, so there is no CI job left to delete. Most catalog repos have no user-facing surface; removing the UI must be cheaper than hand-building one. |
...
| `rename_fork.py` | Not a demo script: the one-pass fork rebrand, listed here because this table is the index of everything in `scripts/`. It rewrites the package name, the console-script name, the `DISPUTES` env prefix, the Terraform `name_prefix` stem and the distribution id in a single simultaneous pass, then renames `src/<package>/`. Stdlib-only and offline like the rest, it prints a plan and writes nothing without `--yes`. See `docs/ADOPTING.md`. |
