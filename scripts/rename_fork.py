...
Rewrites the python package name, the console-script name, the ``DISPUTES`` env-var prefix,
...
Markdown prose alone (pass ``--include-docs`` to sweep it too).
    ...
    # Apply it:
    python scripts/rename_fork.py --package acme_disputes --cli acme-disputes \
        --env-prefix ACME --resource acme-disputes --yes
...
# --------------------------------------------------------------------------------------- #
_OLD_PACKAGE = "disputes_chargebacks_manager"
_OLD_CLI = "disputes_chargebacks_manager"
_OLD_ENV_STEM = "DISPUTES"
_OLD_RESOURCE = "f2-svc"
_OLD_DIST = "disputes-chargebacks-manager"
...
def _rules(args: argparse.Namespace) -> list[tuple[str, str, str]]:
    ...
    return [
        ...
        # The env-var prefix. Matched WITHOUT the trailing underscore so both DISPUTES_PROFILE
...
def main() -> int:
    ...
    parser.add_argument("--cli", required=True, help="new console-script name, e.g. acme-disputes")
    parser.add_argument("--env-prefix", required=True, help="new env-var prefix stem, e.g. ACME")
    parser.add_argument(
        "--resource", required=True, help="new Terraform name_prefix stem, e.g. acme-svc"
    )
    parser.add_argument(
