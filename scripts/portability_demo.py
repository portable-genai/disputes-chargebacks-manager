...
from disputes_chargebacks_manager import (
    config,
)
from disputes_chargebacks_manager.domain import (
    kernel,
)
from disputes_chargebacks_manager.ports import (
    PORT_PROTOCOLS,
)
...
def check_the_trail_leaves_this_codebase_intact() -> str:
    with tempfile.TemporaryDirectory(prefix="portability-") as work:
        ...
        # Line 1 is the anchor header the export commits to; every later line is one record.
        # export_jsonl() returns the RECORD count, so the file is exactly one line longer.
        header, records = lines[0], lines[1:]
        if len(lines) != written + 1 or len(records) != written:
            raise AssertionError("the export is not one anchor header followed by one line each")
        if header.get("format") != EXPORT_FORMAT or "entry_hash" not in header.get("anchor", {}):
            raise AssertionError("the export carries no readable anchor header")
        if any("entry_hash" not in record for record in records):
            raise AssertionError("the export is not self-describing JSON Lines")
        # The header is the witness the chain alone cannot be, so it has to name the head the
        # record lines actually end on: a dropped tail leaves a chain that verifies perfectly.
        if header["anchor"]["entry_hash"] != records[-1]["entry_hash"]:
            raise AssertionError("the anchor header does not name the exported chain head")
