# Runbook: Disputes and Chargebacks Manager (F2)

## Deploy (gcp)
1. `DISPUTES_PROFILE=gcp`, install `.[gcp]`, region `asia-southeast1`.
...
3. Ingress is fronted by IAP/Apigee; the app authenticates the S2S caller fail-closed
   (`DISPUTES_S2S_TOKEN` local, Google OIDC + allowlist secure).
4. **Set `DISPUTES_IAP_AUDIENCE`.** Without it the service starts, stays
...
`DISPUTES_IAP_AUDIENCE` is the IAP-protected resource the assertion must be
...
| Symptom | Cause | Fix |
|---|---|---|
| 503, detail names `DISPUTES_IAP_AUDIENCE` | no audience configured | set it to the protected resource above |
...
| `gcp` IAP assertion | `verified` | authenticated: the signature (against IAP's own key set), the audience (against `DISPUTES_IAP_AUDIENCE`), the expiry and the issuer are all checked before any claim is read |
...
1. **Nobody chose a profile.** `DISPUTES_PROFILE` is absent, so no end-user
...
2. **The `local` profile, chosen deliberately.** The seeded personas are a client-asserted
   ...
   `DISPUTES_S2S_TOKEN` is set. Setting that secret closes the S2S
...
- `add_loopback_exposure_guard` is registered at module scope in `api/app.py`, so it holds under
  `uvicorn disputes_chargebacks_manager.api.app:app --host 0.0.0.0` (what the Dockerfile `CMD`
...
Set `DISPUTES_ALLOW_INSECURE_DEMO=1` to accept the exposure deliberately.
...
`DISPUTES_PROFILE` is read once, in `config.py`, and it has three states:
...
`DISPUTES_S2S_TOKEN`. With routing on and the URL unset, the managed profile REFUSES TO BOOT,
...
`DISPUTES_REVIEW_ROUTING` switches routing, read in three states: unset is on, `true`/`false`
(or `on`/`off`) wins, and an emptied or unrecognised value refuses at boot. Off needs no console,
logs one warning at startup, and every routed dispute outcome reports `review_routing: "off"` with an empty
`review_ref`. A hand-off that fails at request time does not fail the request: the response
...
`DISPUTES_AUDIT_ANCHOR` (read by `audit_anchor_path` in
`config/settings.yaml`) to a file on a DIFFERENT volume from
`DISPUTES_AUDIT_PATH`, ideally writable by a different principal. This is
not decoration:
