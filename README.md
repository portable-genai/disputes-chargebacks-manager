# Disputes and Chargebacks Manager (F2)

Deterministic dispute and chargeback lifecycle, intake and representment for BFSI ops.
...
# open a dispute (eligibility is deterministic; an ineligible rejection routes to review):
disputes_chargebacks_manager open DSP-1001 card_scheme 10.4 42000 SGD 2025-05-01 2025-05-10 --as-of 2025-06-01
disputes_chargebacks_manager abuse DSP-1003 retail R-REFUND 80000 SGD 2025-05-20 2025-05-25 --disputes-90d 9 --prior-abuse 1
disputes_chargebacks_manager intake conv-unauth-001
```
...
the story it is presented with. It is enforced by its own required check
(`.github/workflows/demo-gate.yaml`), so it cannot rot quietly:
...
One env var, `DISPUTES_PROFILE`, selects the adapter family:
...
`DISPUTES_ALLOW_INSECURE_DEMO=1` says otherwise. The guard reads the identity
BINDING to decide that, never a service credential: setting
`DISPUTES_S2S_TOKEN` closes the S2S routes and does not open anything else.
See `docs/runbook.md`.
...
| `pii-kit` | the jurisdiction PII pattern pack the dispute pipeline redacts with before any audit write or model call |
...
**Name the profile.** `DISPUTES_PROFILE` has no default. Leaving it unset is
