...
@pytest.fixture()
def dispute_service(container: Container) -> Any:
    """The disputes orchestrator wired to the real local adapters."""
    return build_service(container)


@pytest.fixture()
def api_client() -> Iterator[TestClient]:
    """The API served to a loopback peer, which is the only posture the local profile allows."""
    from disputes_chargebacks_manager.api.app import app
