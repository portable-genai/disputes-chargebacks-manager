...
from disputes_chargebacks_manager.adapters.gcp.identity import (
    _AUDIENCE_ENV,
...
from disputes_chargebacks_manager.config import Settings
from disputes_chargebacks_manager.ports.identity import (
    VERIFIED,
...
#: A canonical open-dispute body whose reason code is filed past its window, so the deterministic
#: verdict is INELIGIBLE and the response's ``requires_human_review`` is True (a routed rejection).
...
_REBOUND_SETTINGS = "\n".join(
    [
        ...
        *[
        ...
        # The remaining ports bind their LOCAL adapter under both local and gcp here, so a served
        # request runs offline: this suite exercises the IAP identity seam, not the case spine.
        "  case_engine:",
        *[f"    {p}: {_PKG}.adapters.local.case_engine:LocalCaseEngine" for p in ("local", "gcp")],
        f"    onprem: {_PKG}.adapters.onprem.case_engine:OnPremCaseEngine",
        "  document_extraction:",
        *[
            f"    {p}: {_PKG}.adapters.local.document_extraction:LocalDocumentExtractor"
            for p in ("local", "gcp")
        ],
        f"    onprem: {_PKG}.adapters.onprem.document_extraction:OnPremDocumentExtractor",
        "  conversation_channel:",
        *[
            f"    {p}: {_PKG}.adapters.local.conversation_channel:LocalConversationChannel"
            for p in ("local", "gcp")
        ],
        f"    onprem: {_PKG}.adapters.onprem.conversation_channel:OnPremConversationChannel",
        "  regulator_response:",
        *[
            f"    {p}: {_PKG}.adapters.local.regulator_response:LocalRegulatorResponder"
            for p in ("local", "gcp")
        ],
        f"    onprem: {_PKG}.adapters.onprem.regulator_response:OnPremRegulatorResponder",
        "  narration:",
        *[f"    {p}: {_PKG}.adapters.local.narration:LocalNarrator" for p in ("local", "gcp")],
        f"    onprem: {_PKG}.adapters.onprem.narration:OnPremNarrator",
    ]
)
...
def _managed_app(monkeypatch: pytest.MonkeyPatch, tmp_path: Any, audience: str | None) -> Any:
    ...
    supplies it. ``None`` leaves the variable UNSET, which is the unconfigured deployment; an
    EMPTIED variable is a different state and the loader refuses it before any app exists.
    """
...
def test_an_emptied_audience_is_refused_by_the_settings_loader(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Any
) -> None:
    """EMPTIED is not UNSET: ``${VAR:-}`` refuses rather than resolving to the written default.

    An operator who emptied the variable expressed an intent that names nothing, and the loader
    is the last place that still knows a default was written, so it refuses there and names the
    variable. Unset remains the unconfigured deployment tested below, which comes up and refuses
    every caller with a 503; emptied never produces settings at all.
    """
    path = tmp_path / "settings.yaml"
    path.write_text(_REBOUND_SETTINGS + "\n", encoding="utf-8")
    monkeypatch.setenv(_PROFILE_ENV, "gcp")
