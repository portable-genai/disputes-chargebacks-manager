...
from disputes_chargebacks_manager.adapters.local.audit import (
    LocalAuditAdapter,
)
...
_ANCHOR_ENV = "DISPUTES_AUDIT_ANCHOR"
...
def _record_three(settings: Settings) -> LocalAuditAdapter:
    ...
    for dispute in (
        ...
        sample_cases.INELIGIBLE_DISPUTE,
        sample_cases.PII_DISPUTE,
    ):
        service.open_dispute(dispute, actor=sample_cases.ACTOR, as_of=sample_cases.AS_OF)
    return audit
...
def test_an_append_after_truncation_cannot_relaunder_the_anchor(tmp_path: Path) -> None:
    ...
    # Fresh non-audit ports (stateless local adapters), but the SAME truncated audit adapter, so
    # the append is refused before it can re-anchor the store as it now stands.
    container = build_container(settings)
