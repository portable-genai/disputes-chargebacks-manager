...
hosted GitHub Actions check runs the real walkthrough headless (``make demo-selftest``) on every
...
def test_the_demo_data_is_obviously_fictional() -> None:
    disputes = (demo.ROUTINE_DISPUTE, demo.ABUSE_DISPUTE)
    for dispute in disputes:
        assert dispute.id.startswith("DSP-DEMO"), dispute.id + " is not an obvious demo id"
        assert dispute.currency == "SGD"
    blob = " ".join(d.narrative for d in disputes) + demo.ACTOR + demo.PLANTED_NRIC
    for real_looking in ("@gmail.", "@outlook.", "@yahoo."):
        assert real_looking not in blob
    assert ".example" in demo.ACTOR, "the actor should use an .example domain"
    # The planted identifier is a synthetic national id used only to prove redaction happened.
    assert demo.PLANTED_NRIC == "S1234567D"
