"""The provenance the UI's model pill states must be true of the profile the service is running.
...
def test_the_health_contract_carries_both_halves() -> None:
    ...
    """
    from disputes_chargebacks_manager.api.schemas import HealthResponse
...
def test_not_implemented_is_claimed_only_by_an_adapter_that_never_calls_a_model() -> None:
    ...
    if settings.generator_model != "managed-not-implemented":
        return
    from disputes_chargebacks_manager.config import _GENERATOR_PORT
