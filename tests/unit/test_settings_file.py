...
def test_an_unset_env_var_takes_the_files_written_default(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Three-state expansion, the two states that resolve: unset defaults, a value wins."""
...
def test_an_env_var_set_to_empty_is_refused_rather_than_resolved(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """The middle state REFUSES: it neither inherits the default nor resolves to empty.

    Resolving to empty made ``${VAR:-http://audit:8080}`` with ``VAR=""`` indistinguishable from
    ``${VAR:-}``, and for a base URL, an allowlist or a path the empty string is the permissive
    branch. The loader is the last place that still knows a default was written, so it refuses
    here rather than handing a downstream field a value nobody reviewed.
    """
    path = tmp_path / "settings.yaml"
    path.write_text("region: ${TEMPLATE_TEST_REGION:-fallback-region}\n", encoding="utf-8")
