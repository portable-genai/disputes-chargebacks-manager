...
from disputes_chargebacks_manager.adapters._review_payload import (
    result_to_review,
)
...
def test_the_payload_that_reaches_the_wire_is_redacted_whichever_family_built_it() -> None:
    ...
    disposition = DisputeDisposition(
        ...
        summary=f"NRIC {sample_cases.PLANTED_NRIC} filed a dispute that requires sign-off",
        ...
        citations=(
            Citation(
                ...
                snippet=f"NRIC {sample_cases.PLANTED_NRIC}",
            ),
        ),
    )
    payload = repr(
        result_to_review(
            disposition, maker=sample_cases.ACTOR, tenant=sample_cases.TENANT
        ).to_payload()
    )
...
def test_the_whole_pipeline_answers_on_local_and_fails_fast_on_onprem() -> None:
    local = build_service(build_container(local_settings()))
    result = local.open_dispute(
    ...
    assert result.disposition.citations, "an offline answer must still be cited"
    assert result.review_ref, "the ineligible rejection must have been routed (R8)"

    onprem = build_service(build_container(local_settings(profile="onprem")))
    with pytest.raises(NotImplementedError):
        onprem.open_dispute(
            sample_cases.INELIGIBLE_DISPUTE, actor=sample_cases.ACTOR, as_of=sample_cases.AS_OF
        )
