"""FastAPI application for Disputes and Chargebacks Manager (F2).
...
``DISPUTES_PROFILE`` is NO CHOICE rather than a silent ``local``. Every
...
``DISPUTES_S2S_TOKEN`` is wrong in the most dangerous direction:
...
header and no ``X-Dev-Persona`` then got the seeded persona list and a real dispute decision.
...
_ALLOWED_CALLERS_ENV = "DISPUTES_S2S_ALLOWED_CALLERS"
_AUDIENCE_ENV = "DISPUTES_S2S_AUDIENCE"
_API_HOST_ENV = "DISPUTES_API_HOST"
...
def require_service_caller(request: Request) -> None:
    ...
    stays OPEN when ``DISPUTES_S2S_TOKEN`` is unset (loopback dev with zero
    secrets), so an UNSET ``DISPUTES_PROFILE`` must never be allowed to
    ...
    ``DISPUTES_ALLOW_INSECURE_DEMO=1``.
    """
    if not _request_choice(request).service_auth_configured:
        raise HTTPException(
            ...
            detail=(
                ...
                "DISPUTES_PROFILE is not set, so no authentication scheme "
                "has been chosen"
            ),
        )
...
_SERVICE_NAME = "disputes-chargebacks-manager"
...
_APP_TITLE = "Disputes and Chargebacks Manager"
...
#
# Note what is NOT in this expression: DISPUTES_S2S_TOKEN. A service
...
@app.post("/v1/disputes/open", response_model=OpenDisputeResponse, tags=["disputes"])
def open_dispute(
    ...
    return OpenDisputeResponse(
        ...
        reasons=list(result.eligibility.reasons),
        state=result.case.state.value,
        severity=d.severity.value,
        decision=d.decision.value,
        deadlines=[
            DeadlineModel(name=dl.name, due=dl.due.isoformat(), regulatory=dl.regulatory)
            for dl in result.case.deadlines
        ],
...
@app.post("/v1/disputes/abuse", response_model=AbuseResponse, tags=["disputes"])
def assess_abuse(
...
@app.post("/v1/disputes/representment", response_model=RepresentmentResponse, tags=["disputes"])
def representment(
...
@app.post("/v1/disputes/regulator", response_model=RegulatorResponseModel, tags=["disputes"])
def regulator(
