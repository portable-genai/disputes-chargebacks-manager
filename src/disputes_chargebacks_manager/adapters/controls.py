...
(``DISPUTES_REVIEW_ROUTING=off``), the container binds :class:`DisabledReviewRouter` instead of
the profile's class. It satisfies the port and submits nothing, and the container logs the
posture at startup.

**Recording wrapper.** Every caller that hands a result to the router (the open, abuse and
intake routes, their agent tools and CLI commands, each through the dispute service they build)
wraps the bound router in :class:`RecordingReviewRouter` for that one call, so what it returns
can say what happened: ``routed``, ``failed``, ``off`` or ``not_required``. A failure is logged
and absorbed here rather than failing an already-decided, already-audited dispute outcome, but
it is never invisible: the caller reports ``failed`` and an empty reference, which nobody can
mistake for a reviewed result.
"""
...
from ..domain.models import DisputeDisposition
