...
One env var (``DISPUTES_PROFILE``) selects the adapter family for every
...
silently resolving to empty (an operator who emptied a value expressed an intent, and it names
nothing, and for a base URL or an allowlist empty is the permissive branch), and SET-AND-VALID
wins.
"""
...
from .domain.abuse_engine import AbusePolicy
...
_PROFILE_ENV = "DISPUTES_PROFILE"
_SETTINGS_ENV = "DISPUTES_SETTINGS"
...
#: What ``generator_model`` reports under ``local``, and what the local narrator notes as the model
#: that answered. One name for one stub, so the pill reads the same before and after an answer.
LOCAL_STUB_MODEL: str = "deterministic-offline-stub"

#: Profiles that mean "running on managed cloud infrastructure", for the pill's runtime title.
_MANAGED_PROFILES: frozenset[str] = frozenset({"gcp"})

#: The port whose ACTIVE binding decides the model ``generator_model`` reports.
#: Named once here so rebinding it for a profile changes the model pill in the same edit.
_GENERATOR_PORT: str = "narration"
...
def _model_from_settings(settings: object, path: str) -> str:
    ...
    There is deliberately no second, "harder" model a flag could swap in here: a resolver
    that named a model the adapter never read would put a model on screen that never answered.
    """
...
class ProfileChoice:
    ...
    The variable is ``DISPUTES_PROFILE``, named once in
    :data:`_PROFILE_ENV`.
    ...
    ``os.environ.get("DISPUTES_PROFILE", "local")``: that fallback reads an
...
_PKG = "disputes_chargebacks_manager"
...
DEFAULT_BINDINGS: dict[str, dict[str, str]] = {
    ...
    "case_engine": {
        "local": f"{_PKG}.adapters.local.case_engine:LocalCaseEngine",
        "gcp": f"{_PKG}.adapters.gcp.case_engine:CloudCaseEngine",
        "onprem": f"{_PKG}.adapters.onprem.case_engine:OnPremCaseEngine",
    },
    "document_extraction": {
        "local": f"{_PKG}.adapters.local.document_extraction:LocalDocumentExtractor",
        "gcp": f"{_PKG}.adapters.gcp.document_extraction:CloudDocumentExtractor",
        "onprem": f"{_PKG}.adapters.onprem.document_extraction:OnPremDocumentExtractor",
    },
    "conversation_channel": {
        "local": f"{_PKG}.adapters.local.conversation_channel:LocalConversationChannel",
        "gcp": f"{_PKG}.adapters.gcp.conversation_channel:CloudConversationChannel",
        "onprem": f"{_PKG}.adapters.onprem.conversation_channel:OnPremConversationChannel",
    },
    "regulator_response": {
        "local": f"{_PKG}.adapters.local.regulator_response:LocalRegulatorResponder",
        "gcp": f"{_PKG}.adapters.gcp.regulator_response:CloudRegulatorResponder",
        "onprem": f"{_PKG}.adapters.onprem.regulator_response:OnPremRegulatorResponder",
    },
    "narration": {
        "local": f"{_PKG}.adapters.local.narration:LocalNarrator",
        "gcp": f"{_PKG}.adapters.gcp.narration:CloudNarrator",
        "onprem": f"{_PKG}.adapters.onprem.narration:OnPremNarrator",
    },
}
...
def _expand(value: str) -> str:
    ...
    to that helper and there is exactly one implementation of the rule: UNSET takes the written
    default, SET-AND-EMPTY raises, a value wins. ``${VAR}`` with no ``:-`` defaults to the empty
    string, so unset resolves to empty and emptied still raises.
    """
...
def _read_settings_file(path: Path | None = None) -> dict[str, Any]:
    ...
    An EXPLICIT path (argument or ``DISPUTES_SETTINGS``) that does not
...
#: Where the client-owned policy packs are looked for, relative to the process working directory.
DEFAULT_POLICY_PATH = Path("config") / "policy_packs.yaml"


def load_policy(
    path: Path | None = None,
) -> tuple[tuple[ReasonCodePack, ...], AbusePolicy]:
    """Load the reason-code packs and abuse policy from the policy file, or the shipped defaults.

    The numbers are the client's, so they are configuration, not code: this reads
    ``config/policy_packs.yaml`` when present and falls back to
    :data:`~.domain.policy_defaults.DEFAULT_REASON_CODE_PACKS` /
    :data:`~.domain.policy_defaults.DEFAULT_ABUSE_POLICY` when it is not (a wheel with no repo
    checkout around it still works). An EXPLICIT path that is missing raises: somebody named a
    file. Unknown track names and malformed rows fail closed with a clear error.
    """
    target = path if path is not None else DEFAULT_POLICY_PATH
    if not target.exists():
        if path is not None:
            raise FileNotFoundError(f"policy file {target} does not exist")
        return DEFAULT_REASON_CODE_PACKS, DEFAULT_ABUSE_POLICY
    data = yaml.safe_load(target.read_text(encoding="utf-8")) or {}
    if not isinstance(data, dict):
        raise ValueError(f"policy file {target} must contain a mapping at the top level")
    packs = _packs_from(data.get("reason_code_packs", []))
    policy = _abuse_policy_from(data.get("abuse_policy", {}))
    return packs, policy


def _packs_from(raw: object) -> tuple[ReasonCodePack, ...]:
    if not isinstance(raw, list):
        raise ValueError("'reason_code_packs' must be a list")
    packs: list[ReasonCodePack] = []
    for entry in raw:
        if not isinstance(entry, dict):
            raise ValueError("each reason-code pack must be a mapping")
        track = DisputeTrack(str(entry["track"]))  # raises on an unknown track (fail closed)
        rules = tuple(
            ReasonCodeRule(
                code=str(rule["code"]),
                description=str(rule["description"]),
                filing_window_days=int(rule["filing_window_days"]),
                response_deadline_days=int(rule["response_deadline_days"]),
                evidence_required=tuple(str(e) for e in rule.get("evidence_required", [])),
            )
            for rule in entry.get("rules", [])
        )
        packs.append(ReasonCodePack(track=track, scheme=str(entry["scheme"]), rules=rules))
    return tuple(packs)


def _abuse_policy_from(raw: object) -> AbusePolicy:
    if not raw:
        return DEFAULT_ABUSE_POLICY
    if not isinstance(raw, dict):
        raise ValueError("'abuse_policy' must be a mapping")
...
class Settings:
    ...
    #: the shipped illustrative numbers; ``load`` reads ``config/policy_packs.yaml`` when present.
    reason_code_packs: tuple[ReasonCodePack, ...] = field(
        default_factory=lambda: DEFAULT_REASON_CODE_PACKS
    )
    abuse_policy: AbusePolicy = field(default_factory=lambda: DEFAULT_ABUSE_POLICY)
    ...
    def load(cls, path: Path | None = None) -> Settings:
        ...
        settings = cls(
            ...
            case_url=str(data.get("case_url") or ""),
            doc6_url=str(data.get("doc6_url") or ""),
...
class Container:
    ...
    @cached_property
    def case_engine(self) -> CaseEnginePort:
        adapter = self._bind("case_engine")
        assert isinstance(adapter, CaseEnginePort)
        return adapter

    @cached_property
    def document_extraction(self) -> DocumentExtractionPort:
        adapter = self._bind("document_extraction")
        assert isinstance(adapter, DocumentExtractionPort)
        return adapter
    ...
    @cached_property
    def regulator_response(self) -> RegulatorResponsePort:
        adapter = self._bind("regulator_response")
        assert isinstance(adapter, RegulatorResponsePort)
        return adapter

    @cached_property
    def narration(self) -> NarrationPort:
        adapter = self._bind("narration")
        assert isinstance(adapter, NarrationPort)
        return adapter
...
def build_service(container: Container, *, routing: Any = None) -> Any:
    ...
    and the lazy import keeps that graph acyclic and cheap).
    """
    from .domain.dispute_service import DisputeService
    ...
    return DisputeService(
        ...
        reason_code_packs=settings.reason_code_packs,
        abuse_policy=settings.abuse_policy,
    )
